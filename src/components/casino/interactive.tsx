...
function Search({ initialTab, category, onPlay }: { initialTab: string; category: string; onPlay: () => void }) {
  ...
  return <div className="search-content">
    ...
    <div className="search-results" role="tabpanel" aria-label={tab}>
      {tab === 'games' && <><h3>{query ? 'Ergebnisse' : category === 'all' ? 'Beliebt' : categories.find(item => item.id === category)?.title ?? 'Spiele'}</h3>{games.length ? games.map(game => <button className="search-game" key={game.id} onClick={onPlay}><Image src={gameImage(game)} alt="" width={60} height={60} unoptimized /><span><strong>{game.name}</strong><small>{game.provider}</small></span></button>) : <p className="empty-results">Keine Spiele gefunden. Versuche eine andere Suche.</p>}</>}
